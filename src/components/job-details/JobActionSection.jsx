...
export default function JobActionSection({ jobId, isActive, isExpired = false, onToggle }) {
  const navigate = useNavigate()

  const [modalOpen, setModalOpen] = useState(false)
  const [toggling, setToggling]   = useState(false)

  const isReopenDisabled = !isActive && isExpired
        ...
        <div className='px-6 py-5 flex flex-col gap-3'>
          ...
          <p className='text-xs text-gray-400 dark:text-gray-500 leading-relaxed'>
            {isActive
              ...
              : isExpired
                ? 'This job has expired and is closed. Extend the application deadline before reopening.'
