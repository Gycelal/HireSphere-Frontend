...
export default function OtpVerificationPage () {
  ...
  const handleSubmit = async e => {
    ...
    setLoading(true)
    try {
      const res = await publicApi.post('accounts/verify-email/', {
        otp: otp,
  ...
  const handleResend = async () => {
    ...
    setResending(true)
    try {
      const res = await publicApi.post('accounts/resend-otp/', {
        user_id: state.user_id
      })
      if (res.status === 200) {
          ...
          >
            {digits.map((digit, i) => (
              <input
                ...
                className={`
                  flex-1 min-w-0 aspect-square max-w-13
      ...
      <div className='flex items-center justify-center gap-1.5 text-sm'>
        ...
        {canResend ? (
          <button
            ...
            disabled={resending}
            className='font-semibold text-violet-600 dark:text-violet-400 hover:text-violet-700 dark:hover:text-violet-300 disabled:opacity-50 transition-colors duration-200 cursor-pointer'
          >
