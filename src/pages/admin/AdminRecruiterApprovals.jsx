...
function StatusBadge({ status }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold border ${styles[status] ?? styles.pending}`}>
      <span className="material-symbols-outlined text-[0.85rem]">{icons[safeStatus] ?? "schedule"}</span>
...
function RecruiterCell({ row }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <span className="font-medium text-gray-900 dark:text-white">{row.first_name +" "+ row.last_name}</span>
    </div>
  );
}
...
export default function AdminRecruiterApprovalsPage() {
  ...
  try {
    let orderingValue;
    if (sort === "newest") orderingValue = "-date_joined";
    else if (sort === "oldest") orderingValue = "date_joined";
    else if (sort === "name_asc") orderingValue = "first_name";
    else if (sort === "name_desc") orderingValue = "-first_name";

    const paramsObj = { page, ordering: orderingValue };
