...
const FindJobs = () => {
  ...
  useEffect(() => {
    ...
    if (filtersChanged) {
      ...
      if (page !== 1) {
        setPage(1);
        return; // Abort this fetch; the setPage(1) will trigger a new render and fetch
      }
    }
    ...
    try {
      const params = {
        ...
        ...(experienceRequired && {
          ...
          ...(experienceRequired[1] !== 0 && {
            experience_required_max: experienceRequired[1],
          }),
        }),
      };
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:items-center">
          ...
          <div className="flex flex-wrap gap-4 items-center">
            ...
            {EXPERIENCE_REQUIRED.map((exp) => (
              <label
                key={exp.label}
                className="flex items-center gap-2 cursor-pointer text-gray-600 dark:text-gray-400 font-normal"
              >
                <input
                  type="checkbox"
        ...
        {jobs.length === 0 ? (
          ...
          <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {jobs.map((job) => (
              <li
                key={job.id}
                className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between"
              >
                <div>
                ...
                <div className="mt-5 flex justify-end">
                  <Link
                    ...
                    className="text-violet-600 hover:text-violet-700 dark:text-violet-400 dark:hover:text-violet-300 font-medium text-sm flex items-center gap-1 transition-colors"
                  >
                    View Details{" "}
